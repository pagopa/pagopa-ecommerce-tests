...
class NotificationErrorPendingTransactionTests(
  ...
  @Test
  fun `Should perform refund for NPG transaction stuck in NOTIFICATION_ERROR status with KO send payment result outcome`() {
    // pre-conditions
      ...
      IntegrationTestData(
        ...
        view =
          TransactionTestUtils.transactionDocument(
            TransactionStatusDto.NOTIFICATION_ERROR, ZonedDateTime.now()),
  ...
  @Test
  fun `Should expire transaction stuck in NOTIFICATION_ERROR status with OK send payment result outcome without writing event to DLQ`() {
    // pre-conditions
    val testTransactionId = getProgressiveTransactionId()
    val npgActivationData = TransactionTestUtils.npgTransactionGatewayActivationData()
    (npgActivationData as NpgTransactionGatewayActivationData).correlationId =
